    ...
    for (i, order_a) in enumerate(orders)
        for (j, order_b) in enumerate(orders)
            if order_a != order_b
                ...
                # println(sailing_time)
                push!(adj[i], Edge(
                    i, 
                    j,
                    order_a.profit + order_b.profit -sailing_time * BUNKER_COST, 
                    [order_b.sailing_time + sailing_time])) # from, to, cost, res_usage (currently bunker usage)
            end
        end
    end
    # for order in orders
...
function Label(source,V)
    ...
    v[source] = 1
    return Label(source,0,v,0,[0],missing)
end
...
function extend(e::Edge,λ::Label,v::Int64, Edges::Vector{Edge}, res_max)
    ...
    unreachable = λ.s
    if all(λ.res_usage+e.res_usage .> res_max)
        return Nothing
    end
    ...
    return Label(v,unreachable,visited,λ.cost+e.cost,λ.res_usage+e.res_usage,λ)
end
